...
class MultiBotManager:
    def __init__(self, root: tk.Tk, sys_logger: logging.Logger):
        ...
        if icon_path:
            try:
                ...
                self.sys_logger.warning(
                    f"Could not load application icon: {e}"
                )
        ...
        lbl_txt = (
            f"[{hk_clk}] Target Click   "
            f"[{hk_cht}] Target Chat   "
            f"[{hk_tgl}] Start/Stop   "
            f"[{hk_bos}] Boss Key   "
            f"[{hk_kil}] KILL ALL"
        )
        ...
        self.hotkey_thread = NativeHotkeyManager(
                ...
                try:
                    if wtype in (
                        ...
                        if wtype != "Label":
                            widget.configure(
                                selectcolor=t["bg_m"],
                                activebackground=t["bg_s"],
                                activeforeground=t["fg_m"],
                            )
    ...
    def auto_save_profiles(self):
        try:
            profiles = {
                f"Core_{i}": bot.get_profile_data()
        ...
        try:
            ...
            if profiles:
                ...
                self.sys_logger.info(
                    "Previous session auto-loaded successfully."
                )

        except Exception as e:
            self.sys_logger.error(
                f"Auto-load failed: {e}"
            )
    ...
    def show_tutorial(self):
        ...
        tk.Label(
            tut_win,
            text="STREAM BOT - USER GUIDE & PRO FEATURES",
            font=("Consolas", 11, "bold"),
            bg=t_bg,
            fg=t_fg,
        ).pack(
...
Spintax allows you to create dynamic, randomized chat messages so your bot never sends the exact same message twice.
...
Type messages in the table.

For example: 10.5 mins.

Exactly 10 minutes and 30 seconds after you click START, that exact message will be sent with priority.
...
When enabled, the bot takes a screenshot of the live stream every 30-60 seconds, sends it to OpenAI or Gemini, and generates a realistic reaction message.

7. STEALTH MODE / BACKGROUND OPERATION

When enabled, the bot sends clicks and text directly into the application's background code so you can use the mouse for other things.
"""
    ...
    def check_admin(self):
        if not is_admin():
            ...
            tk.Label(
                notice_frame,
                text=(
                    "NOTICE: Run this app as Administrator "
                    "if games/windows block the bot's inputs."
                ),
                font=("Consolas", 9, "bold"),
                bg="#780000",
                fg="#ffffff",
    ...
    def load_profiles(self):
        try:
            ...
            self.sys_logger.info(
                f"Profiles loaded from {file_path}"
            )
    ...
    def route_hotkey(
        ...
        if action == "target_click":
            bot.set_click_target()

        elif action == "target_chat":
            bot.set_type_target()
        ...
        elif action == "pause_click":
            bot.toggle_click_pause()
    ...
    def on_closing(self):
        ...
        try:
            ...
            self.sys_logger.error(
                f"Auto-save during shutdown failed: {e}"
            )
